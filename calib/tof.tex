...
% A few paddles that had not been previously knocked out had drifts on either the resolution or the mean $\Delta$ TOF and were not properly calibrated. Table~\ref{tabletobeadded} shows which paddles should be knocked out due to this. Figure~\ref{good paddle} shows an example of a good paddle's resolution and mean $\Delta$ TOF while figure~\ref{bad paddle} shows and example of a bad paddle. The TOF resolution for every paddle by run is stored in the g12 wiki.
