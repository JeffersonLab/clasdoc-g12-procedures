...
Calibrating the TOF system provides timing for charged tracks, which is then used for particle identification. The TOF was calibrated in accordance to CLAS's standard procedure. For g12, the procedure was as follows~\cite{clas.thesis.bookwalter}: 
\begin{enumerate}
	\item Counter status: %identify and eliminate dead channels.
	\item ADC pedestals: %record values of each ADC channel when no tracks are passing through the TOF.
	\item TDC linearization constants: %using fits to pulser data, determine the conversion function from TDC channels to nanoseconds.
	\item Time-walk corrections: %correct for the instrumental shift in times created by pulses of varying height passing through a leading-edge discriminator. This is typically done by fits to data taken with a laser illuminating each phototube in sync with a pulser so that the timing response over a wide range of ADC values can be probed. However, by the time the g12 run was commissioned, the CLAS TOF laser system had largely fallen into disrepair. Many alternatives were tested, and the system that gave the best timing resolution for g12 was to perform the time-walk calibration on channels with the appropriate laser data, and to use constants from prior runs for channels where no such data existed.
	...
	\item Left-right delay constants: %determine corrections required for the differences in timing between photomultipliers at each end of the same paddle.
	\item Attenuation length: %determine and record for each paddle the average length over which the scintillation light is attenuated by half.
	\item Minimum-ionizing particle pulse heights: %for each paddle, determine and record the ADC channel number for minimum-ionizing particles.
	\item Effective velocity: %for each paddle, determine the effective velocity of light, which can vary as the plastic ages and becomes less transparent.
	\item Paddle-to-paddle offsets: %correct for shifts in the relative timing between paddles due to cable lengths or other factors.
\end{enumerate}

The following plots are for g12's run number 56855, pass0 v7. They did not change in pass1. 
